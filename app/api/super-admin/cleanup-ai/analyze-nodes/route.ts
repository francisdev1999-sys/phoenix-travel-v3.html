...
import { analyzeNode, estimateCost } from '@/lib/cleanup/analyzer';
...
export async function POST(req: NextRequest) {
  ...
  const allCandidates = runNodeRules();
  ...
  for (const candidate of candidates) {
    try {
      const result = await analyzeNode(candidate);
      ...
      console.error(`Node analysis failed for ${candidate.id}:`, err);
    }
  }
