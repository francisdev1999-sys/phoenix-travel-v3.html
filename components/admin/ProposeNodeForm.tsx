...
const CATEGORIES = ['Ancient Civilizations', 'Lost Technology', 'Cosmic Events', 'Hidden History',
                    'Sacred Geometry', 'Consciousness', 'Interdimensional', 'Prophecy'];
...
export default function ProposeNodeForm({ onSuccess, onCancel }: Props) {
  const [form, setForm] = useState({
    label: '', category: 'Ancient Civilizations', description: '',
    ...
    <div className="flex flex-col gap-5">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2 mt-1.5">
          {CATEGORIES.map(c => {
            const cc = (CATEGORY_COLORS as Record<string, string>)[c] ?? '#94a3b8';
            return (
      ...
      <Field label="Description *" value={form.description} onChange={v => set('description', v)} placeholder="Full description of this node..." textarea />
      ...
      {/* Claims */}
      <ListField label="Key claims" items={claims} setItems={setClaims} placeholder="A specific claim..." />

      {/* Criticisms */}
      <ListField label="Criticisms" items={criticisms} setItems={setCriticisms} placeholder="A counter-argument..." />

      {/* Open questions */}
      <ListField label="Open questions" items={openQuestions} setItems={setOpenQuestions} placeholder="An unresolved question..." />

      <Field label="Mainstream view" value={form.mainstreamView} onChange={v => set('mainstreamView', v)} placeholder="How do mainstream scholars interpret this?" textarea />
